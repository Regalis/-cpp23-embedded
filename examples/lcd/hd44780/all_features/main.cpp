...
#include "clocks.hpp"
#include "drivers/lcd/hd44780/common.hpp"
...
int main()
{
    ...
    using lcd_t = hd44780::hd44780<hd44780::interface_for<descriptor>,
                                   configuration,
                                   with_animations_custom_delay<20>,
    ...
    while (true) {
        ...
        auto length = lcd.animate_puts("Hello world :)", line::line_1, alignment::center);
        timer::delay(500ms);

        lcd.animate_puts("C++23 bare metal", line::line_2, alignment::center);
        timer::delay(500ms);
        lcd.animate_puts("by Regalis", line::line_3, alignment::center);

        timer::delay(1s);

        // smooth clear whole line
        lcd.animate_clear_line(line::line_3);

        timer::delay(1s);

        // back to the second line
        lcd.animate_puts("[ brightness ]", line::line_3, alignment::center);
        ...
        lcd.animate_clear_line(line::line_3);
        timer::delay(500ms);
        length = lcd.animate_puts("[custom characters]", line::line_3, alignment::center);

        const auto show_custom_character = [&](const auto& character) {
            auto _ = timer::scoped_delay(500ms);
            lcd.cursor_goto(static_cast<uint8_t>(configuration.columns / 2) - 1, line::line_4);
        ...
        lcd.clear_line(line::line_4);
        lcd.animate_clear_line(line::line_3);

        lcd.animate_puts("Happy hacking!", line::line_3, alignment::center);
        timer::delay(1s);
        lcd.animate_clear_line(line::line_3);
        timer::delay(1s);
        lcd.animate_puts("Drop a follow <3", line::line_3, alignment::center);
        timer::delay(1s);
        lcd.animate_clear_line(line::line_3);
        timer::delay(1s);
        lcd.animate_puts("Twitter/X:", line::line_3, alignment::center);
        timer::delay(500ms);
        lcd.animate_puts("@regalis_tech", line::line_4, alignment::center);
